  ...
  public function viewsForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!empty($this->view->result) && !empty($action_options)) {
      ...
      foreach ($this->view->result as $row_index => $row) {
        ...
        foreach ($radio_choices as $i => $rc) {
          $bulk_form_key[$i] = self::calculateEntityBulkFormKey(
            $entity,
            $row->{$base_field},
            $rc
          );
        ...
        $checked = $bulk_form_key[1];// isset($this->tempStoreData['list'][$bulk_form_key]);
        if ($checked) {
        ...
        $form[$this->options['id']][$row_index] = [
          ...
          '#default_value' => $bulk_form_key[1], // $checked,
          '#id' => 'ID__'. $row_index,
          '#options' => [
            $bulk_form_key[0] => $radio_choices[0],
            $bulk_form_key[1] => $radio_choices[1],
            $bulk_form_key[2] => $radio_choices[2]],
    ...
    if ($form_state->get('step') == 'views_form_views_form') {
      ...
      if (!$form_state->getValue('select_all')) {
        ...
        foreach ($form_state->getValue($this->options['id']) as $row_index => $bulkFormKey) {
          if ($bulkFormKey) {
            // #radio_button: get the button choice that will be sent to the
            // action into the configuration
            $dataFromRadio = $this->getListItem($bulkFormKey, $form[$this->options['id']][$row_index]['#title']);
            if ($dataFromRadio) {
              $this->tempStoreData['list'][$bulkFormKey] = $dataFromRadio;
              $this->tempStoreData['configuration']['id-action'][$dataFromRadio[0]] = $dataFromRadio[4];
            }
          }
          else {
